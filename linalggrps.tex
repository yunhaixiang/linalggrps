...
\begin{proposition}Let $V=R^{2n}$ be a free $R$-module of rank $2n$, let $e$ be an ordered basis of $V$, and let $B:V\times V\rightarrow R$ be an orthogonal form, then
    \begin{enumerate}
        \item $B(x,y)=[x]_{e}^T [B]_e [y]_e$,
        \item $[B]_e^{\mathrm{T}}=-[B]_e$ and $[B]_e$ has zero diagonal, 
        \item  $[B]_e$ is invertible,
        \item $[B]_{Pe}=[P]^{\mathrm{T}}_e[B]_e[P]_e$ for any invertible linear map $P:V\rightarrow V$.
