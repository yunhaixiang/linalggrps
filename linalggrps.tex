...
\theoremstyle{remark}
  ...
  \newenvironment{mathremark}[1][]%
    {\refstepcounter{theorem}\par\medskip\noindent
     \textbf{\if\relax\detokenize{#1}\relax\else#1 \fi\thetheorem. }\ignorespaces}%
    {\par\medskip}
...
\captionsetup[table]{position=above,font=normalfont,labelfont=bf}
...
\begin{mathremark}[Localization of Modules]
    Here is the content.
\end{mathremark}
...
\begin{example}Let $V$ be a free module over $R$ and $q:V\rightarrow R$ a quadratic form. Here are some more examples of algebraic groups over $R$. 
    \begin{enumerate}
        ...
        \[\mathrm{GPin}_{V,q}:\mathbf{Alg}_R\rightarrow\mathbf{Grp}\]
        where 
