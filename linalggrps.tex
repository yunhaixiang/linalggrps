...
\begin{example}Here are some examples of equivalent categories
    \begin{enumerate}
        ...
        \item The category of complex representations of a finite group $G$ (whose morphisms are equivariant maps) is equivalent to the category of $\mathbb C[G]$-modules, and they are also equivalent to $\mathrm{Fun}(\mathrm BG,\mathbf{Vect}_{\mathbb C})$, where $\mathrm B G$, called the \textit{delooping groupoid} of $G$, is the category with a single object $\bullet$, with the endomorphisms of $\bullet$ being group elements, and with the group operation as its composition. 
        \item The category of $G$-sets for a group $G$
        \item The category of topological spaces
    \end{enumerate}
    It is an easy exercise for the reader to spot what the equivalences in the cases above are.
\end{example}
...
\begin{definition}Let $\EuScript C$ be a locally small category. Define 
    ...
    as its \textit{categories of presheaves and copresheaves} respectively.
    ...
    \end{aligned}\]
    where $(f\circ -):\mathrm{Hom}(X,Y)\rightarrow \mathrm{Hom}(X,Z)$ is the same as in \Cref{def:hom-functors}. Dually, the \textit{co-Yoneda embedding} is the contravariant functor 
    \[\begin{aligned}
    ...
    \end{aligned}\]
    where $(-\circ f):\mathrm{Hom}(Z,X)\rightarrow \mathrm{Hom}(Y,X)$ is the same as in \Cref{def:hom-functors}. 
