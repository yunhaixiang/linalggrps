...
\begin{example}Let $V=R^n$ be the free module of rank $n$ over a ring $R$. The standard inner product $\langle\cdot,\cdot\rangle:V\times V\rightarrow R$ given by $\langle v,w\rangle=v_{1}w_{1}+\cdots+v_nw_n$ for each $v,w\in V$, is an orthogonal form.
\end{example}
\begin{example}Let $V=R^n$ be the free module of rank $n$ over a ring $R$. The \textit{standard orthogonal form} of signature $(p,q)$ on $V$, where $p,q\in\mathbb N$ such that $p+q=n$, is 
    ...
    where $b_{n,0}$ is just the standard inner product. 
\end{example}
...
\begin{definition}
    ...
    \[\sigma:E\rightarrow E\qquad x\mapsto -b-\omega\]
...
\end{proposition}
\begin{remark}
    Let $E$ be a quadratic étale $R$-algebra, then we can write each element of $E$ uniquely as $a+b\omega$ where $a,b\in R$ with $\omega$ as in \Cref{def:quadetale}. Let $X\in \mathrm{M}_{n,m}(E[x_1,\dots,x_\ell])$, define $(X)_{E/R}$ as the ideal in $R[x_1,\dots,x_\ell]$, generated by the entries of $A,B\in \mathrm M_n(R[x_1,\dots,x_\ell])$ where $X=A+B\omega$. 
\end{remark}
...
\begin{example}Let $E$ be a quadratic étale $R$-algebra with involution $\sigma:E\rightarrow E$, $V$ a free $E$-module of rank $n$, and $h:V\times V\rightarrow E$ a $\sigma$-Hermitian form. Here are some more examples of algebraic groups over $R$. 
    \begin{enumerate}
        ...
        \[\widetilde{\mathrm{U}_{V,h}}=\mathrm{Spec}\left(\frac{R[X,Y][y]}{((X+Y\omega)^*[h]_e(X+Y\omega)-[h]_e,y\det(X+Y\omega)-1)_{E/R}}\right)\]
        ...
        where for indeterminant matrices $X=(x_{i,j})_{1\le i,j\le n}$ and $Y=(y_{i,j})_{1\le i,j\le n}$ 
        \[\widetilde{\mathrm{SU}_{V,h}}=\mathrm{Spec}\left(\frac{R[X,Y]}{((X+Y\omega)^*[h]_e(X+Y\omega)-[h]_e,\det(X+Y\omega)-1)_{E/R}}\right)\]
