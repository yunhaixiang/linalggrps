...
\begin{proposition}\label{prop:gram}Let $V=R^n$ be the free module of rank $n$ over a ring $R$ with an ordered basis $e$. Suppose $B:V\times V\rightarrow R$ is a form. Let $\sigma:R\rightarrow R$ be an involution. We have the following. 
    \begin{enumerate}
        \item $[B]_e^{*}=[B]_e$ iff $B$ is $\sigma$-symmetric, given $B$ is $\sigma$-sesquilinear,
        \item $[B]_e^{\mathrm{T}}=[B]_e$ iff $B$ is symmetric, given $B$ is bilinear,
        \item $[B]_e^{\mathrm{T}}=-[B]_e$ iff $B$ is bilinear and alternating, given $\frac{1}{2}\in R$,
        ...
        \item $B$ is nondegenerate iff $[B]_e$ is invertible, given $B$ is bilinear or $\sigma$-sesquilinear,
    \end{enumerate}
    where $M^*:=(\sigma(m_{j,i}))\in M_{k\times \ell}(R)$ denotes the $\sigma$-conjugate transpose of $M=(m_{i,j})\in M_{\ell\times k}(R)$.
    \begin{proof}We will only prove
        \begin{enumerate}
            \item a
        \end{enumerate}
    \end{proof}
...
\begin{example}Let $V$ be a free $R$-module of rank $n$ with an orthogonal form $B:V\times V\rightarrow R$. Here are some more examples of algebraic groups over $R$.
    \begin{enumerate}
        ...
        \[\widetilde{\mathrm{O}_{V,B}}=\mathrm{Spec}\left(\frac{R[X]}{(X^T[B]_eX-[B]_e)}\right)\]
        ...
        where for an indeterminant matrix $X=(x_{i,j})_{1\le i,j\le n}$ 
        \[\widetilde{\mathrm{SO}_{V,B}}=\mathrm{Spec}\left(\frac{R[X]}{(X^T[B]_eX-[B]_e, \det X-1)}\right)\]
        ...
        where for each $R$-algebra $S$ 
        \[\mathrm{GO}_{V,B}(S)=\{g\in \mathrm{Aut}_{\mathbf{Mod}_S}(V_S):\exists\lambda\in S^\times,\forall v,w\in V,\,B_S(gv,gw)=\lambda B_S(v,w)\}\]
        where for an indeterminant matrix $X=(x_{i,j})_{1\le i,j\le n}$ 
        \[\widetilde{\mathrm{GO}_{V,B}}=\mathrm{Spec}\left(\frac{R[X][y,\lambda,\nu]}{(X^T[B]_eX-\lambda [B]_e, y\det(X)-1,\lambda\nu-1)}\right)\]
