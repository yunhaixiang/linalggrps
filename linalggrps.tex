...
\end{definition}
\begin{convention}
    Let $S$ a $R$-algebra. If $M$ is a $R$-module, we also use the notation $M_S=M\otimes_R S$. If $M$ is an $S$-module, we write $M_R=\mathrm{Res}_{S/R} M$. 
\end{convention}
\begin{convention}Let $X=(x_{i,j})_{1\le i,j\le n}$ be a $n$-by-$n$ matrix of indeterminants. For any ring $R$, let
    ...
    Abusing notation, identify $X=(x_{i,j})_{1\le i,j\le n}\in M_n(R[X])$. For any $F=(f_{i,j})_{1\le i,j\le n}\in M_n(R[X])$, denote $\det F:=\sum_{\sigma\in \mathrm S_n}\mathrm{sgn}(\sigma)\prod_{i=1}^n f_{i,\sigma(i)}\in R[X]$ and $\mathrm{tr}(A):=\sum_{i=1}^n f_{i,i}\in R[X]$. 
    Moreover, we denote the ideal $(F):=(f_{1,1},f_{1,2},\dots,f_{n,n})\subseteq R[X]$. 
\end{convention}
\begin{example}\label{eg:alggroup}Here are some more examples of algebraic groups over $R$,
\begin{enumerate}
