...
\begin{enumerate}
	\item a class of \textit{objects}, also denoted as $\EuScript C$
	\item for each pair of objects $X,Y\in \EuScript C$, a class $\mathrm{Hom}_{\EuScript C}(X,Y)$ of \textit{morphisms} from $X$ to $Y$, where we call $X$ the \textit{domain} (or \textit{source}) of $f$ and $Y$ the \textit{codomain} (or \textit{target}) of $f$
...
\end{enumerate}
\end{definition}
\begin{convention}We use the notation $f:X\rightarrow Y$ to mean $f\in \mathrm{Hom}_{\EuScript C}(X,Y)$.
\end{convention}
\begin{convention} When the category $\EuScript C$ is clear from context, we write $\mathrm{Hom}(X,Y)$ for $\mathrm{Hom}_{\EuScript C}(X,Y)$.
\end{convention}
\begin{remark}It is an easy exercise to show the identity morphism for an object is unique.
\end{remark}
\iffalse
\begin{remark}We view the domain and codomain of a morphism $f:X\rightarrow Y$ to be part of the data of the morphism. Some authors require that a category have so called \textit{domain and codomain class functions}, but we aviod this as it makes the definition cumbersome. Alternatively, we could simply assume that $\mathrm{Hom}(X,Y)$ are disjoint for distinct pairs of objects $X,Y\in\EuScript C$, but this is unimportant set-theoretic detail that the reader is justified to ignore. 
\end{remark}\fi
...
\end{definition}
\begin{definition}Suppose $\EuScript C,\EuScript D$ are categories, their product category $\EuScript C\times \EuScript D$ is the category where objects are pairs $(X,Y)$ where $X\in \EuScript C$ and $Y\in \EuScript D$, and morphisms \[\mathrm{Hom}((X_1,Y_1),(X_2,Y_2))=\mathrm{Hom}(X_1,X_2)\times \mathrm{Hom}(Y_1,Y_2)\]
...
\begin{definition}
    ...
    For each $f:X\rightarrow Y$ in $\EuScript C$, we denote by $f^{\mathrm{op}}:Y\rightarrow X$ the corresponding \textit{opposite morphism} in $\EuScript C^{\mathrm op}$
\end{definition}
\begin{definition}
...
\end{definition}
\begin{example}Let $\EuScript C$ be a category, then there is the \textit{identity functor} $\mathrm{id}_{\EuScript C}:\EuScript C\rightarrow \EuScript C$ for $\EuScript C$, which maps each object $X\in\EuScript C$ by $\mathrm{id}_{\EuScript C}(X)=X$ and each morphism $f:X\rightarrow Y$ by $\mathrm{id}_{\EuScript C}[f]=f$. 
\end{example}
...
\begin{definition}Suppose $\EuScript C$ is locally small and $X\in\EuScript C$. The \textit{hom-functor} of $X$ is the functor
    \[\mathrm{Hom}(X,-):\EuScript C\rightarrow \mathbf{Set}\quad\quad
        Y\mapsto \mathrm{Hom}(X,Y)\quad\quad
        (f:Y\rightarrow Z)\mapsto (f\circ -)
    \] 
    where $(f\circ -):\mathrm{Hom}(X,Y)\rightarrow \mathrm{Hom}(X,Z)$ maps $g\mapsto f\circ g$. There is also a contravariant version 
    \[\mathrm{Hom}(-,X):\EuScript C^{\mathrm{op}}\rightarrow \mathbf{Set}\quad\quad
        Y\mapsto \mathrm{Hom}(Y,X)\quad\quad
        (f^{\mathrm{op}}:Z\rightarrow Y)\mapsto (-\circ f)
    \] 
    where $(-\circ f):\mathrm{Hom}(Z,X)\rightarrow \mathrm{Hom}(Y,X)$ maps $g\mapsto g\circ f$. We also define the functor 
    \[\mathrm{Hom}(-,-):\EuScript C^{\mathrm{op}}\times \EuScript C\rightarrow \EuScript D\quad\quad (X,Y)\mapsto \mathrm{Hom}(X,Y)\]
    and for a morphism $(f^{\mathrm{op}}:X_1\rightarrow X_2,g:Y_1\rightarrow Y_2):(X_1,Y_1)\rightarrow (X_2,Y_2)$, we map it to 
    \[(g\circ -\circ f):\mathrm{Hom}(X_1,Y_1)\rightarrow \mathrm{Hom}(X_2,Y_2)\quad\quad h\mapsto g\circ h\circ f\]
\end{definition}
...
\begin{example}Let $\EuScript C,\EuScript D$ be categories and $\mathscr F:\EuScript C\rightarrow \EuScript D$ a functor, then there is a \textit{identity natural transformation} $\mathrm{id}_{\mathscr F}:\mathscr F\Rightarrow\mathscr F$ given by mapping $(\mathrm{id}_{\mathscr{F}})_X=\mathrm{id}_{\mathscr F(X)}$ for each $X\in\EuScript C$. 
\end{example}
...
\begin{example}Here are some examples of equivalent categories
    \begin{enumerate}
        \item $\mathbf{Ab}\simeq \mathbf{Mod}_{\mathbb Z}$
        \item $\mathbf{Gal}_{L/K}^{\mathrm{op}}\simeq \mathbf{SubGrp}_{\mathrm{Gal}(L/K)}$ where $\mathbf{Gal}_{L/K}$ is the category of intermediate extensions of a Galois extension $L/K$ with injections as morphisms, and $\mathbf{SubGrp}_{\mathrm{Gal}(L/K)}$ is the category of subgroups of $\mathrm{Gal}(L/K)$ with injective homomorphisms as morphisms. 
        \item $\mathbf{Top}\simeq \mathbf{Kur}$ where $\mathbf{Kur}$ is the category where objects are sets $X$ with a Kuratowski closure operator $\mathrm{cl}:2^{X}\rightarrow 2^X$ and morphisms are maps preserving the closure.  
    \end{enumerate}
\end{example}
\begin{definition}Suppose $\EuScript C, \EuScript D$ are categories and $\mathscr F:\EuScript C\rightarrow \EuScript D$ is a functor. 
    For each $X,Y\in\EuScript C$, let
    \[\mathscr F_{X,Y}:\mathrm{Hom}(X,Y)\rightarrow \mathrm{Hom}(\mathscr F(X),\mathscr F(Y))\quad \mathrm{where}\quad f\mapsto \mathscr F[f]\]
    then we say that $\mathscr F$ is 
    \begin{enumerate}
        \item \textit{faithful} if $\mathscr F_{X,Y}$ is injective for all $X,Y\in\EuScript C$,
        \item \textit{full} if $\mathscr F_{X,Y}$ is surjective for all $X,Y\in\EuScript C$,
        \item \textit{fully faithful} if $\mathscr F_{X,Y}$ is bijective for all $X,Y\in\EuScript C$,
    \end{enumerate}
...
\begin{definition}Let $\EuScript C$ be a locally small category. Define $\widehat{\EuScript C}:=\mathrm{Fun}(\EuScript C^{\mathrm{op}},\mathbf{Set})$ and $\widecheck{\EuScript C}:=\mathrm{Fun}(\EuScript C,\mathbf{Set})$ as its \textit{categories of presheaves and copresheaves}.
    The \textit{Yoneda embedding} is the functor
    \[\mathscr H_{\bullet}:\EuScript C\rightarrow \widehat{\EuScript C}\quad\quad X\mapsto \mathscr H_X=\mathrm{Hom}(-,X)\]
    and for $f:Y\rightarrow Z$, define $(\mathscr H_{\bullet}[f])_X:\mathrm{Hom}(X,Y)\rightarrow \mathrm{Hom}(X,Z)$ as $(f\circ -)$ for each $X\in \EuScript C$. 
    Dually, the \textit{co-Yoneda embedding} is the contravariant functor 
    \[\mathscr H^{\bullet}:\EuScript C^{\mathrm{op}}\rightarrow \widecheck{\EuScript C}\quad\quad X\mapsto \mathscr H^X=\mathrm{Hom}(X,-)\]
    and for $f^{\mathrm{op}}:Z\rightarrow Y$, define $(\mathscr H_{\bullet}[f])_X:\mathrm{Hom}(Z,X)\rightarrow \mathrm{Hom}(Y,X)$ as $(-\circ f)$ for each $X\in \EuScript C$. 
    
\end{definition}


\begin{definition}
    Let $\EuScript C$ be a locally small category. Define the \textit{global sections functor} as
    ...
    we define the \textit{global sections cofunctor}, abusing notation, as
...
\begin{lemma}[Yoneda]
    ...
    \begin{proof}
        Let $X\in\EuScript C$ and let $\mathscr F:\EuScript C\rightarrow\mathbf{Set}$ be a functor. For a natural transformation $\varphi:\mathscr{H}^X\rightarrow\mathscr{F}$ and a morphism $f:X\rightarrow Y$ in $\EuScript C$, the diagram given by
        \[
        ...
        \end{tikzcd}\]
        commutes. Thus $\varphi_Y\circ (f\circ -)=\mathscr{F}[f]\circ \varphi_X$. Evaluating at $\mathrm{id}_X$ on both sides yields 
        ...
        \[\Phi:\mathrm{Hom}_{\widecheck{\EuScript C}}(\mathscr H^{\bullet},{-})\Rightarrow \Gamma(\bullet,-)\qquad \Phi_{(X,\mathscr F)}(\varphi)=\varphi_X(\mathrm{id}_X)\]
        and, by the same identity, the inverse of $\Phi$ is naturally
        \[\Psi:\Gamma(\bullet,-)\Rightarrow \mathrm{Hom}_{\widecheck{\EuScript C}}(\mathscr H^{\bullet},{-})\qquad \Psi_{(X,\mathscr F)}(u)=\varphi\]
        where $\varphi_Y:\mathrm{Hom}(X,Y)\rightarrow \mathscr{F}(Y)$ is given by $\varphi_Y(f)=(\mathscr{F}[f])(u)$, for each $Y\in \EuScript C$. It is not hard for the reader to verify that indeed $\Psi\circ \Phi=\mathrm{id}_{\mathrm{Hom}_{\widecheck{\EuScript C}}(\mathscr H^\bullet, -)}$ and $\Phi\circ \Psi=\mathrm{id}_{\Gamma(\bullet,-)}$. 
    \end{proof}
\end{lemma}

\begin{remark} The preceding \Cref{thm:yoneda} is known as the \textit{fundamental theorem of category theory}. 
\end{remark}
...
\begin{theorem}\label{thm:yff}Let $\EuScript C$ be a locally small category, then the Yoneda and co-Yoneda embeddings 
    \[\mathscr{H}_\bullet: \EuScript{C}\rightarrow \widehat{\EuScript C}\quad\mathrm{and}\quad \mathscr{H}^\bullet: \EuScript{C}^{\mathrm{op}}\rightarrow \widecheck{\EuScript C}\]
    are fully faithful. 
    \begin{proof}
        Let $X,Y\in\EuScript C$, then by \Cref{thm:yoneda}, for any functor $\mathscr{F}:\EuScript C\rightarrow \mathbf{Set}$, there is a map 
        \[\Phi_{(X,\mathscr{F})}:\mathrm{Hom}_{\widecheck{\EuScript C}}(\mathscr H^X, \mathscr F)\rightarrow \mathscr{F}(X)\]
        which, by \Cref{rmk:isom}, is a bijection, since $\Phi$ is a natural isomorphism. Let $\mathscr{F}=\mathscr H^Y$, then
        \[\Phi_{(X,\mathscr{H}_Y)}:\mathrm{Hom}_{\widecheck{\EuScript C}}(\mathscr H^X, \mathscr H^Y)\rightarrow \mathscr{H}^Y(X)=\mathrm{Hom}_{\EuScript C}(Y,X)\]
        is a bijection. This gives the inverse for the map
        \[\mathrm{Hom}_{\EuScript C^{\mathrm{op}}}(X,Y)\xrightarrow{\mathscr{H}^\bullet} \mathrm{Hom}_{\widecheck{\EuScript C}}(\mathscr H^X, \mathscr H^Y)\]
        Thus $\mathscr{H}^\bullet$ is fully faithful. For the dual $\mathscr H_\bullet$, the proof is completely symmetric. 
    \end{proof}
\end{theorem}

\begin{lemma}\label{lem:funcpres}Let $\EuScript C,\EuScript D$ be categories and $\mathscr F:\EuScript C\rightarrow\EuScript D$ a functor. Let $\mathscr{D}:\EuScript I\rightarrow \EuScript C$ be a commutative diagram in $\EuScript C$, then $\mathscr F\circ \mathscr D:\EuScript I\rightarrow \EuScript D$ is a commutative diagram in $\EuScript D$. 
    \begin{proof}Straightforward by definition. 
    \end{proof}
\end{lemma}
\begin{lemma}\label{lem:isom}Let $\EuScript C,\EuScript D$ be categories and $\mathscr F:\EuScript C\rightarrow\EuScript D$ a fully faithful functor, then the morphism  $\mathscr F[f]:\mathscr F(X)\rightarrow \mathscr{F}(Y)$ is an isomorphism in $\EuScript D$ if and only if $f:X\rightarrow Y$ is an isomorphism in $\EuScript C$. 
    \begin{proof}
        Forward direction is an easy consequence of \Cref{lem:funcpres} applied to the diagram
        \[\begin{tikzcd}[column sep=3em] % try 6em, 8em, 12em, or even 3cm
        X
        \arrow[rightarrow, r, bend left=30, "f"{yshift=2pt}]
        & Y
        \arrow[rightarrow, l, bend left=30, "f^{-1}"{yshift=-2pt}]
        \end{tikzcd}
        \]
    For the backwards direction, by fullness of $\mathscr F$, choose $g:Y\rightarrow X$ with $\mathscr{F}[g]=\mathscr{F}[f]^{-1}$. Then we have $\mathscr{F}[f\circ g]=\mathscr{F}[f]\circ \mathscr{F}[g]=\mathrm{id}_{\mathscr{F}(X)}$ so by faithfulness of $\mathscr F$, we have $f\circ g=\mathrm{id}_X$. By similar arguments, we have $g\circ f=\mathrm{id}_Y$, so $g$ is the inverse of $f$.  
    Therefore $f$ is an isomorphism in $\EuScript C$. 
    \end{proof}
\end{lemma}
\begin{theorem}Let $\EuScript C$ be a locally small category, then for a morphism $f:X\rightarrow Y$ in $\EuScript C$, TFAE
    \begin{enumerate}
    ...
    \end{enumerate}
    \begin{proof}
        Striaghtforward by \Cref{thm:yff} and \Cref{lem:isom}. 
    \end{proof}
\end{theorem}
...
Throughout the text, unless specified otherwise, fix a ring $R$, which we shall call our \textit{ground ring}. 
\begin{convention}
...
For readers who have experience in algebraic geometry, you might have been taught that an affine $K$-variety for some algebraically closed field $K$ is a subset of $K^n$ for some $n$ given by the vanishing set of some polynomials $f_1,\dots,f_m\in K[x_1,\dots,x_n]$, that is, the set
...
Similarly, for polynomials $f_1,\dots,f_m\in R[x_1,\dots,x_n]$, the set $X(S)$ for an $R$-algebra $S$ can be identified as the set 
...
\end{remark}
\iffalse
\begin{remark}Recall that an algebra is \textit{reduced} if it does not have any nonzero nilpotent elements. 
\end{remark}
\begin{definition}An \textit{affine $R$-variety} is the spectrum of a finitely generated reduced $R$-algebra. The category $\mathbf{AVar}_R$ of affine $R$-varieties is the full subcategory of affine $R$-varieties of $\mathbf{Aff}_R$. 
\end{definition}
\fi
...
Define the contravariant functor \[\mathrm{Spec}:\mathbf{Alg}_R^{\mathrm{op}}\rightarrow \mathbf{Aff}_R\qquad A\mapsto \mathrm{Spec}(A)\]
    where for each homomorphism $\varphi:A\rightarrow B$ of $R$-algebras, there is the induced natural transformation \[\varphi^*:\mathrm{Spec}(B)\rightarrow\mathrm{Spec}(A)\] called the \textit{pullback} given by 
...
\begin{proof}Suppose $A$ is a $R$-algebra, then applying \Cref{thm:yoneda}, namely, the Yoneda lemma
    ...
    Therefore $\mathrm{Spec}$ and $\mathcal O$ are quasi-inverses of each other. 
\end{proof}
\end{theorem}
...
\end{convention}
\begin{definition}Let $S$ a $R$-algebra. If $M$ is an $R$-module, define the \textit{base change} (or \textit{extension of scalars}) of $M$ from $R$ to $S$ as the $S$-module $M\otimes_R S$. If $M$ is a $S$-module, define its \textit{Weil restriction} (or \textit{restriction of scalars}) from $S$ to $R$ as the $R$-module $\mathrm{Res}_{S/R} M$ with the same additive group as $M$ with scalar multiplication given by $rm=\varphi(r)m$ for $r\in R$ and $m\in M$ where $\varphi:R\rightarrow S$ is the structure map. These define functors $(-)\otimes_{R}S:\mathbf{Mod}_{R}\rightarrow \mathbf{Mod}_{S}$ and $\mathrm{Res}_{S/R}:\mathbf{Mod}_{S}\rightarrow \mathbf{Mod}_{R}$. 
    We define the base change and Weil restrictions for algebras in the exact same way, giving rise to functors $(-)\otimes_{R}S:\mathbf{Alg}_{R}\rightarrow \mathbf{Alg}_{S}$ and $\mathrm{Res}_{S/R}:\mathbf{Alg}_{S}\rightarrow \mathbf{Alg}_{R}$. 
\end{definition}
\begin{convention}
    Let $S$ a $R$-algebra. If $M$ is a $R$-module, we also use the notation $M_S=M\otimes_R S$. If $M$ is an $S$-module, we sometimes abuse notation and write $M=\mathrm{Res}_{S/R} M$. 
\end{convention}
\begin{convention}Let $M=(m_{i,j})_{1\le i,j\le n}$ be a $n$-by-$n$ matrix of indeterminants. For any ring $R$, let
    \[R[M]:=R[m_{1,1},m_{1,2},\dots,m_{n,n}]\]
    Abusing notation, identify $M=(m_{i,j})_{1\le i,j\le n}\in M_n(R[M])$. For any $A=(a_{i,j})_{1\le i,j\le n}\in M_n(R[M])$, denote $\det A:=\sum_{\sigma\in \mathrm S_n}\mathrm{sgn}(\sigma)\prod_{i=1}^n a_{i,\sigma(i)}\in R[M]$ and $\mathrm{tr}(A):=\sum_{i=1}^n a_{i,i}\in R[M]$. 
    Moreover, we denote the ideal $(A):=(a_{1,1},a_{1,2},\dots,a_{n,n})\subseteq R[M]$. 
\end{convention}
\begin{example}\label{eg:alggroup}Here are some examples of algebraic groups over $R$,
\begin{enumerate}
    ...
    where $\widetilde{\mathbb G_m}\cong \mathrm{Spec}(R[x,x^{-1}])$,
    ...
    \[\mathrm{GL}_V:\mathbf{Alg}_R\rightarrow\mathbf{Grp}\qquad S\mapsto \mathrm{Aut}_{\mathbf{Mod}_S}(V\otimes_{R}S)\]
    where $\displaystyle\widetilde{\mathrm{GL}_V}\cong \mathrm{Spec}\left(\frac{R[M][y]}{(y\,\mathrm{det}(M)-1)}\right)$ where $M=(m_{i,j})_{1\le i,j\le n}$ is an indeterminant matrix,
    ...
    where $\displaystyle\widetilde{\mathrm{SL}_V}\cong \mathrm{Spec}\left(\frac{R[M]}{(\mathrm{det}(M)-1)}\right)$ where $M=(m_{i,j})_{1\le i,j\le n}$ is an indeterminant matrix.
    \end{enumerate}
\end{example}
...
\begin{definition}Let $V$ be a module over an arbitrary ring $R$. Suppose $\sigma:R\rightarrow R$ is a ring map satisfying $\sigma\circ\sigma=\mathrm{id}$, which we call the \textit{involution} map. A function
    \[B:V\times V\rightarrow R\]
    is called \textit{form} on $V$, and is said to be
    \begin{enumerate}
...
\begin{example}Let $V=R^n$ be the free module of rank $n$ over a ring $R$ with basis $e_1,\dots,e_n$. The standard inner product $\langle\cdot,\cdot\rangle:V\times V\rightarrow R$ given by $\langle v,w\rangle=v_{1}w_{1}+\cdots+v_nw_n$ for each $v,w\in V$, where $v=v_1e_1+\cdots+v_ne_n$ and $w=w_1e_1+\cdots+w_ne_n$, is an orthogonal form on $V$. 
\end{example}
...
\begin{example}Recall that a quadratic form on a free module $V=R^n$ of rank $n$ over $R$ is a quadratic homogeneous polynomial map $q:V\rightarrow R$ 
    ...
    and we say $q$ is nonsingular if $b_q$ is nondegenerate. Suppose $2$ is invertible in $R$, then we can define the polarization $B_q=\frac{1}{2}b_q$ of $q$, which satisfies $q(x)=B_q(x,x)$. 
    Therefore, when $2$ is invertible in $R$, a quadratic form $q$ corresponds uniquely to a symmetric bilinear form $B_q$. 
\end{example}
\begin{example}Let $V=R^{2n}$ be the free module of rank $2n$ over a ring $R$, the form $\omega:V\times V\rightarrow R$ given by $\omega(x,y)=x^{\mathrm T}Jy$, where $J\in M_{2n}(R)$ is the matrix 
    \[J=\begin{pmatrix}0&I_n\\ -I_n& 0\end{pmatrix}\]
    with $I_n$ the $n$-by-$n$ identity matrix, is a symplectic form called the \textit{standard symplectic form}. 
\end{example}

\begin{definition}
    A \textit{quadratic étale} algebra $E$ over a ring $R$ is a $R$-algebra such that \[E\cong R[x]/(x^2+bx+c)\]
    for some $b,c\in R$ where $b^2-4c\in R^\times$. Define its \textit{involution} as the unique nontrivial automorphism 
    \[\sigma:E\rightarrow E\qquad x\mapsto -b-x\]
    which satisfies $\sigma\circ \sigma=\mathrm{id}_E$ and fixes $R$. 
\end{definition}
\begin{example}Let $E$ be a quadratic étale algebra over $R$ with involution $\sigma:E\rightarrow E$. 
\end{example}


\begin{example}scaled Hermitian trace form
\end{example}
\begin{proposition}Let $B:V\times V\rightarrow R$ be a bilinear form. If $B$ is alternating then it is antisymmetric, i.e. $B(x,y)=-B(y,x)$ for $x,y\in V$. 
    Conversely, if $B$ is antisymmetric and $2\in R^\times$, then $B$ is alternating. 
    \begin{proof}Suppose $B$ is alternating. For $v,w\in V$, we have 
        \[0=B(v+w,v+w)=B(v,v)+B(v,w)+B(w,v)+B(w,w)=B(v,w)+B(w,v)\]
        Thus $B(v,w)=-B(w,v)$ for all $v,w\in V$. Conversely, suppose $2\in R^\times$ and $B$ is antisymmetric then for all $v\in V$ we have $B(v,v)=-B(v,v)$ so $2B(v,v)=0$, thus $B(v,v)=0$. 
    \end{proof}
\end{proposition}

\begin{proposition}A finite rank free module admitting a symplectic form is of even rank. All symplectic forms are isometric. 
    \begin{proof}Let $V$ be a finite rank free module over a ring $R$ with symplectic form $P:V\times V\rightarrow R$. 
    \end{proof}
\end{proposition}
\begin{definition}Let $V=R^n$ be the free module of rank $n$ over a ring $R$ with an ordered basis $e=(e_1,\dots,e_n)$. 
    Let $B:V\times V\rightarrow R$ be a form, then the \textit{Gram matrix} of $B$ with respect to ordered basis $e$, is the matrix $[B]_e:=(B(e_i,e_j))_{1\le i,j\le n}\in M_n(R)$. 
\end{definition}
\begin{proposition}\label{prop:gram}Let $V=R^n$ be the free module of rank $n$ over a ring $R$ with an ordered basis $e$. Suppose $B:V\times V\rightarrow R$ is a form. Let $\sigma:R\rightarrow R$ be an involution. We have the following. 
    \begin{enumerate}
        \item $[B]_e^{*}=[B]_e$ iff $B$ is $\sigma$-sesquilinear and $\sigma$-symmetric,
        \item $[B]_e^{\mathrm{T}}=[B]_e$ iff $B$ is bilinear and symmetric,
        \item $[B]_e^{\mathrm{T}}=-[B]_e$ iff $B$ is bilinear and alternating,
        \item $B(x,y)=[x]_e^{*} [B]_e[y]_e$ for all $x,y\in V$, iff $B$ is $\sigma$-sesquilinear,
        \item $B(x,y)=[x]_e^{\mathrm{T}} [B]_e[y]_e$ for all $x,y\in V$, iff $B$ is bilinear,
        \item $[B]_{Pe}=[P]^{*}_e[B]_e[P]_e$ for any invertible linear map $P:V\rightarrow V$, iff $B$ is $\sigma$-sesquilinear,
        \item $[B]_{Pe}=[P]^{\mathrm{T}}_e[B]_e[P]_e$ for any invertible linear map $P:V\rightarrow V$, iff $B$ is bilinear,
        \item $B$ is nondegenerate iff its Gram matrix $[B]_e$ is invertible,
    \end{enumerate}
...
\begin{definition}Let $R$ be an arbitrary ring, suppose $S$ is a $R$-algebra, $V$ is a module over $R$, and $B:V\times V\rightarrow R$ a bilinear form on $V$. 
    ...
    \iffalse
    Moreover, suppose $\sigma:R\rightarrow R$ is an involution on $R$ and $\tau:S\rightarrow S$ is an involution on $S$, satisfying $\tau\circ \varphi=\varphi\circ \sigma$ where $\varphi:R\rightarrow S$ is the structure map. The \textit{base change} of a $\sigma$-sesquilinear form $H:V\times V\rightarrow R$ on $V$ from $R$ to $S$ is the $\tau$-sesquilineaer form 
    \[H_S:V_S\times V_S\rightarrow S\qquad (v\otimes r,w\otimes s)\mapsto H(v,w)\tau(r)s\]
    on $V_S$, extended $\sigma$-sesquilinearly. 
    \fi
\end{definition}
\begin{definition}Base change for 
\end{definition}
...
\begin{example}Let $V$ be a free $R$-module of rank $n$ with an orthogonal form $B:V\times V\rightarrow R$. Here are some more examples of algebraic groups over $R$.
    \begin{enumerate}
        ...
        \[\mathrm{O}_{V,B}(S)=\{g\in \mathrm{Aut}_{\mathbf{Mod}_S}(V_S):\forall v,w\in V,\,B_S(gv,gw)=B_S(v,w)\}\]
        where $\displaystyle\widetilde{\mathrm{O}_{V,B}}=\mathrm{Spec}\left(\frac{R[M]}{(M^TBM-B)}\right)$ where $M=(m_{i,j})_{1\le i,j\le n}$ is an indeterminant matrix
        \item the \textit{special orthogonal group} of the pair $(V,B)$
        \item the \textit{orthogonal semilitude group}
        \item the \textit{special orthogonal semilitude group}
    \end{enumerate}
\end{example}



\begin{example}Let $V$ be a free $R$-module of rank $2n$ with an symplectic form $P:V\times V\rightarrow R$. Here are some more examples of algebraic groups over $R$.
    \begin{enumerate}
        \item the \textit{symplectic group} of the pair $(V,P)$
        \[\mathrm{Sp}_{V,P}:\mathbf{Alg}_R\rightarrow \mathbf{Grp} \]
        where for each $R$-algebra $S$ 
        \[\mathrm{Sp}_{V,P}(S)=\{g\in \mathrm{Aut}_{\mathbf{Mod}_S}(V_S):\forall v,w\in V,\,B_S(gv,gw)=B_S(v,w)\}\]
        where $\displaystyle\widetilde{\mathrm{Sp}_{V,P}}=\mathrm{Spec}\left(\frac{R[M]}{(M^TBM-B)}\right)$ where $M=(m_{i,j})_{1\le i,j\le n}$ is an indeterminant matrix
        \item the \textit{symplectic semilitude group}
    \end{enumerate}
\end{example}
\begin{example}Unitary groups
\end{example}
\begin{example}Pin group, Spin group, GPin, GSpin
\end{example}

\begin{example}Exceptional groups 
\end{example}
